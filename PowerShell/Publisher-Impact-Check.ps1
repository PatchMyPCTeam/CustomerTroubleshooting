...
function Write-Result {
    ...
    if ($Win32AppIds -and $Win32AppIds.Count -gt 0) {
        Write-Host "`t- Affected Win32 App IDs:"
...
function Test-Scenario {
    param(
        ...
        [hashtable]$AppOnlyProducts,
        [hashtable]$UpdateOnlyProducts,
    ...
    switch ($Id) {
        ...
        5 { return (Test-Scenario5 -Settings $Settings -AppOnlyProducts $AppOnlyProducts -UpdateOnlyProducts $UpdateOnlyProducts) }
        6 { return (Test-Scenario6 -PublishingHistory $PublishingHistory -AppOnlyProducts $AppOnlyProducts -UpdateOnlyProducts $UpdateOnlyProducts) }
        default {
...
function Test-Scenario5 {
    ...
    $IntuneUpdates = foreach ($Product in $Settings.Updates.SearchPattern) {
        if ($AppOnlyProducts.Keys -contains $Product.ProductId) {
            $Product.ProductId
        }
    }

    $IntuneApps = foreach ($Product in $Settings.Applications.SearchPattern) {
        if ($UpdateOnlyProducts.Keys -contains $Product.ProductId) {
            $Product.ProductId
        }
    }
...
function Test-Scenario6 {
    ...
    $PublishedApps    = $PublishingHistory | Where-Object { $_.Operation -like 'IntuneApp*' } | ForEach-Object {
        Add-Member -InputObject $_ -MemberType NoteProperty -Name 'ProductName' -Value $_.Title -PassThru
    }
    $PublishedUpdates = $PublishingHistory | Where-Object { $_.Operation -like 'IntuneUpdate*' } | ForEach-Object {
        Add-Member -InputObject $_ -MemberType NoteProperty -Name 'ProductName' -Value ($_.Title -replace 'Update for ') -PassThru
    }

    $ErroneouslyPublishedPkgs = foreach ($App in $PublishedApps) {
        if ($UpdateOnlyProducts.Values -contains $App.ProductName) {
            [PSCustomObject]@{
                ProductName   = $App.ProductName
                PublishedDate = $App.Date
                Win32AppId    = $App.Win32AppId
                Tenant        = $App.TenantId
            }
        }
    }
    ...
    $ErroneouslyPublishedPkgs += foreach ($Update in $PublishedUpdates) {
        if ($AppOnlyProducts.Values -contains $Update.ProductName) {
            [PSCustomObject]@{
                ProductName   = $Update.ProductName
                PublishedDate = $Update.Date
                Win32AppId    = $Update.Win32AppId
    ...
    if (([array]$ErroneouslyPublishedPkgs).Count -gt 0) {
        Write-Log -Message ('Test-Scenario6: Impacted. Erroneously published packages: {0}' -f ($ErroneouslyPublishedPkgs.Win32AppId | ConvertTo-Json))
        return $ErroneouslyPublishedPkgs.Win32AppId
    }
    else {
...
if ($BackupRestoreDate -ne $false -and $BackupRestoreDate -is [datetime]) {
    ...
    if (-not (Test-Path $PublishingHistoryCsv)) {
        ...
        try {
            ...
            $Scenario6Result = Test-Scenario -Id 6 -PublishingHistory $PublishingHistory -AppOnlyProducts $AppOnlyProducts -UpdateOnlyProducts $UpdateOnlyProducts
        }
        catch {
